...
This requires to select the appropriate USB stick and the correct baud rate. (for IMST iM871A-USB, this is 57600 baud). For heat cost allocators and most battery operated devices, the bus mode is usually T.

![HWSetup](doc/adapter_config_settings.png)

### AES keys

The keys identifiers are a combination of the manufacturer code and the device ID (e.g. AAA-12345678). The key needs to be entered usually as hex values with 32 digits. There can be devices that require 96 digits.

![AESKeys](doc/adapter_config_keys.png)

The easiest way to setup the keys is to connect without setting it and for every encrypted device with unknown key, an entry with "UNKNOWN" key is generated by the adatper. Then you can fill in the corresponding key and save the settings. If you see devices you don't know or just want to get rid of (devices from neighbors), you can enter them in the blocked devices tab.
