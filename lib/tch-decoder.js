...
function parseHCA(data, version) {
    ...
    if (version == 0x94) {
        ...
        temp1 = data.readUInt16LE(11) / 100;
        temp2 = data.readUInt16LE(13) / 100;
    } else if (version == 0x69) {
        if (data.length < 14) {
            return false;
        }
        temp1 = data.readUInt16LE(10) / 100;
        temp2 = data.readUInt16LE(12) / 100;
    } else {
        temp1 = false;
        temp2 = false;
    }
    ...
    if (temp1 && temp2) {
        dr.push(fakeMBUS(temp1 - temp2, 'K', 'VIF_TEMP_DIFF', 'Temperature Difference'));
    }
