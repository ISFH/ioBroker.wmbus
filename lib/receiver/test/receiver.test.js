...
const mockPort = '/dev/mockPort';
const options = { baudRate: 38400 };
...
async function initDevice(deviceClass, mode, port) {
    ...
    receiver = new ReceiverClass(port, options, mode, onMessage, console.log, showReceiverLogMessages ? logger : emptyLogger);
    await receiver.init();
}
...
function initMockery(receiverClass) {
    ...
    mockery.registerAllowables(['./SerialDevice', './DeviceMock', '@serialport/stream', 'fs',
        'stream', 'util', 'debug', './node.js', 'tty', 'util', 'supports-color', 'os', 'tty',
        'has-flag', './common', 'ms', '@serialport/binding-mock', '@serialport/binding-abstract',
        'debug', 'debug', './legacy', 'events', './SimpleLogger', '../CulReceiver', 'constants',
        '../AmberReceiver', '../AmberMessage', './AmberMessage', '../ImstReceiver', '../SimpleReceiver',
        './HciMessage', '../HciMessage', '../EbiReceiver', './EbiMessage', '../EbiMessage']);
}
