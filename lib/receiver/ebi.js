...
class EBI {
    ...
    logDebug(msg) {
        this.logFunc.debug('EBI: ' + msg);
    }

    logError(msg) {
        this.logFunc.error('EBI: ' + msg);
    }
    ...
    __On_ebi_data__(data) {
        ...
        if (that.__parser_buffer__.length >= that.__parser_length__) {
            const emitBuffer = that.__parser_buffer__.slice(0, that.__parser_length__);
            that.__parser_buffer__ = that.__parser_buffer__.slice(that.__parser_length__);
            that.__parser_length__ = -1;

            //this.logDebug(emitBuffer);
    ...
    __Read_ebi_package__(message_id, callback) {
        const that = this;
        const message_id_read = message_id | 0x80;
        ...
        that.__read_callbacks__.push(function(data) {
            if (!Buffer.isBuffer(data)) {
                ...
                const chkRead = data[data.length - 1];
                data[data.length - 1] = 0;
                const chk = that.__Checksum__(data);
                if (chk != chkRead) {
                    that.logError(data.toString('hex'));
                    callback && callback(undefined, { message: 'Incorrect checksum' });
                    return;
                }

                const return_id = data[2];
    ...
    __Default_ebi_callback__(data) {
        ...
        if (!Buffer.isBuffer(data)) {
            ...
            if (return_id == 0xE0) { //Received data
                ...
                this.logError(payload.toString('hex'));
            }
        }
    }

    dataReceived(payload) { // eslint-disable-line no-unused-vars
        this.logError('No data handling for incoming packets defined!');
    }
    ...
    __Send_ebi_command__(message_id, payload, callback) {
        ...
        // a payload
        const esb_present = (payload.length ? true : false);

        // create a EBI_v1p0 frame
        const pkg = that.__Build_Ebi_package__(message_id, payload);
    ...
    Device_Information(callback) {
        const message_id = 0x01;
        this.__Send_ebi_command__(message_id, callback);
    }

    Device_State(callback) {
        const message_id = 0x04;
        this.__Send_ebi_command__(message_id, callback);
    }

    Reset(callback) {
        const message_id = 0x05;
        const that = this;
        that.__Send_ebi_command__(message_id, function(res) { // eslint-disable-line no-unused-vars
            // wait for second response (device information)
    ...
    Firmware_version(callback) {
        const message_id = 0x06;
        this.__Send_ebi_command__(message_id, callback);
    }

    Restore_factory_default_settings(callback) {
        const message_id = 0x07;
        this.__Send_ebi_command__(message_id, callback);
    }

    Save_settings(callback) {
        const message_id = 0x08;
        this.__Send_ebi_command__(message_id, callback);
    }
    ...
    Physical_address_set(address, callback) {
        ...
        const payload = Buffer.from(address);
        this.__Send_ebi_command__(message_id, payload, callback);
    }
    ...
    Network_stop(callback) {
        const message_id = 0x30;
        this.__Send_ebi_command__(message_id, callback);
    }

    Network_start(callback) {
        const message_id = 0x31;
        this.__Send_ebi_command__(message_id, callback);
    }
...
class EBI_WMBUS extends EBI {
    ...
    Operating_channel_set(channel, callback) {
        ...
        const message_id = 0x11;
        if (channel in this.CHANNELS_WMB) {
            const payload = Buffer.from([this.CHANNELS_WMB[channel]]);
            this.__Send_ebi_command__(message_id, payload, callback);
        } else {
            this.__Send_ebi_command__(message_id, callback);
        }
    ...
    Network_role_set(role, callback) {
        ...
        const payload = Buffer.from([this.NETWORK_ROLE_WMB[role]]);
        this.__Send_ebi_command__(message_id, payload, callback);
    }
    ...
    Network_automated_settings_set(creation, callback) {
        ...
        const payload = Buffer.from([creation << 7 | 0x00, creation << 15 | 0x00]);
        this.__Send_ebi_command__(message_id, payload, callback);
    }
    ...
    Network_preferences_set(network_preference, callback) {
        ...
        const payload = Buffer.from([network_preference]);
        this.__Send_ebi_command__(message_id, payload, callback);
    }
    ...
    network_security(enable_security, key_shared, use_encryption, update_key, network_key, callback) {
        ...
        if ((typeof update_key !== 'undefined') && (typeof network_key !== 'undefined')) {
            const key = Buffer.from(network_key);
    ...
    init(dev, opts, mode) {
        ...
        that.Device_Information(function(res) {
            that.logDebug('Found ' + that.DEVICE_INFORMATION_PROTOCOL[res.payload[0]] + ' protocol and module ' + that.DEVICE_INFORMATION_MODULE[res.payload[1]]);
            if (!(res.payload[0] & 0x40)) {
                that.logError('Error: This does not seem to be an Embit Wirless M-Bus device!');
                all_okay = false;
                return;
            }
            ...
            that.Reset(function(res)  {
                if (res.payload[0] != 0x10) {
                    that.logError('Warning device not ready! ' + res.payload.toString('hex'));
                    all_okay = false;
                }
                that.logDebug('Device ready');
                // set channel, power, energy saving
                that.Output_power_set(0x0F, function(res) {
                    that.logDebug('Power set to max');
                    ...
                    that.Operating_channel_set(channel, function(res) {
                        that.logDebug('Channel set to ' + chTxt);
                        if (res.transmission_ok != 'Success') { all_okay = false; }
                        that.Energy_save_set(0x00, 0x00, function(res) {
                            that.logDebug('Energy saving disabled');
                            if (res.transmission_ok != 'Success') { all_okay = false; }
                            // start network
                            if (all_okay) { // autocreate network
                                that.Network_automated_settings_set(1, function(res) {
                                    that.logDebug('Automatically start network');
                                    if (res.transmission_ok != 'Success') { all_okay = false; }
                                    if (all_okay) // save settings
                                    {
                                        that.Save_settings(function(res) { // eslint-disable-line no-unused-vars
                                            that.logDebug('Settings saved');
                                            that.Network_start(function(res) {
                                                that.logDebug('Network start '  + (res.transmission_ok != 'Success' ? 'failed!' : 'okay!'));
                                            });
                                        });
                                    } else {
                                        that.Network_start(function(res) {
    ...
    dataReceived(payload) {
        //this.logDebug("Telegram received");

        const opts = payload[0] * 0x100 + payload[1];
        let rssi = false;
        let ts = false;
        let frame_type = 'A';
        let l_field = false;
        ...
        let m_field = 0;
        let manufacturer_id = ''; // eslint-disable-line no-unused-vars
        let a_field = 0;
        let a_field_id = ''; // eslint-disable-line no-unused-vars
        let a_field_ver = 0; // eslint-disable-line no-unused-vars
        let a_field_type = 0; // eslint-disable-line no-unused-vars
        if (opts & 0x0001) { // address
            m_field = payload.readUInt16LE(i);
            manufacturer_id = String.fromCharCode((m_field >> 10) + 64) + String.fromCharCode(((m_field >> 5) & 0x1f) + 64) + String.fromCharCode((m_field & 0x1f) + 64); // eslint-disable-line no-unused-vars
            i += 2;
            a_field = payload.slice(i, i+6);
            a_field_id = a_field.readUInt32LE(0).toString(16); // eslint-disable-line no-unused-vars
            i += 4;
            a_field_ver = payload[i++]; // eslint-disable-line no-unused-vars
            a_field_type = payload[i++]; // eslint-disable-line no-unused-vars
        }

        if (!l_field || !c_field || !m_field || !a_field) {
            this.logError('Telegram with incomplete data link layer received!');
        }
