 ...
 *
 * Implementation of the IMST HCI Interface as described here:
 * https://wireless-solutions.de/downloads/Radio-Modules/iM871A/WMBus_HCI_Spec_V1_6.pdf
 *
 */
...
class CRC {
    ...
    calc(data) {
        ...
        data.forEach(function(n) {
            const val = ((n & 0x80) >> 7) | ((n & 0x40) >> 5) | ((n & 0x20) >> 3) | ((n & 0x10) >> 1) | ((n & 0x08) << 1) | ((n & 0x04) << 3) | ((n & 0x02) << 5) | ((n & 0x01) << 7);
            chk = that.table[((chk >> 8) ^ val) & 0xFF] ^ (chk << 8);
        });
        chk ^= this.finalXor;
        chk &= 0xFFFF;
        let n = chk & 0xFF;
        const clow = ((n & 0x80) >> 7) | ((n & 0x40) >> 5) | ((n & 0x20) >> 3) | ((n & 0x10) >> 1) | ((n & 0x08) << 1) | ((n & 0x04) << 3) | ((n & 0x02) << 5) | ((n & 0x01) << 7);
        n = (chk >> 8) & 0xFF;
        const chigh = ((n & 0x80) >> 7) | ((n & 0x40) >> 5) | ((n & 0x20) >> 3) | ((n & 0x10) >> 1) | ((n & 0x08) << 1) | ((n & 0x04) << 3) | ((n & 0x02) << 5) | ((n & 0x01) << 7);
        return (chigh << 8) | clow;
    }
}
...
class IMST_WMBUS {
    ...
    logDebug(msg) {
        this.logFunc.debug('IMST: ' + msg);
    }

    logError(msg) {
        this.logFunc.error('IMST: ' + msg);
    }
    ...
    buildPayloadPackage(endpoint_id, msg_id, crc, payload) {
        ...
        const res = Buffer.alloc(4 + payload.length + 2);
        ...
        payload.copy(res, 4);
        if (crc) {
            const csum = that.calcChecksum(res);
    ...
    readPayloadPackage(msg_id, callback) {
        ...
        that.readCallbacks.push(function(data) {
            if (!Buffer.isBuffer(data) || (data[0] != that.CMD_START)) {
                ...
                const control = data[1] >> 4;
                const end_id = data[1] & 0x0F;
                const tsField = (control & 0b0010 ? true : false);
                const rssiField = (control & 0b0100 ? true : false);
                const crcField = (control & 0b1000 ? true : false);
                const return_id = data[2];
                const payloadlength = data[3];
                let pos = payloadlength + 4;
                const ts = (tsField ? data.readUInt32LE(pos) : new Date().getTime());
                if (tsField) { pos += 4; }
                const rssi = (rssiField ? (8.0/15.0 * data[pos++]) - 380.0/3.0  : -1);
                const crc = (crcField ? data.readUInt16BE(pos++) : 0);
                const payload = data.slice(4, payloadlength + 4);
        ...
        that.readTimeouts.push(setTimeout(function() {
            ...
            callback && callback(undefined, { message: 'Timeout waiting for response to message id ' + msg_id });
        }, 3000));
    }
    ...
    sendPackage(msg_id, payload, callback) {
        ...
        const pkg = that.buildPayloadPackage(this.DEVMGMT_ID, msg_id, true, payload);
        that.logDebug(`TX: ${pkg.toString('hex')}`);
    ...
    onData(data) {
        ...
        if (that.parserLength == -1) {
            ...
            const control = that.parserBuffer[1] >> 4;
            const add = (control & 0b0010 ? 4 : 0) + (control & 0b0100 ? 1 : 0) + (control & 0b1000 ? 2 : 0);
        ...
        if (that.parserBuffer.length >= that.parserLength) {
            ...
            that.parserLength = -1;
            that.logDebug(`RX: ${emitBuffer.toString('hex')}`);
    ...
    defaultCallback(data) {
        ...
        if ((typeof that.incomingData === 'function') && (data[0] == that.CMD_START) && ((data[1] & 0x0F) == that.RADIOLINK_ID)) { // telegram received
            const control = data[1] >> 4;
            const end_id = data[1] & 0x0F; // eslint-disable-line no-unused-vars
            const tsField = (control & 0b0010 ? true : false);
            const rssiField = (control & 0b0100 ? true : false);
            const crcField = (control & 0b1000 ? true : false);
            const return_id = data[2]; // eslint-disable-line no-unused-vars
            const payloadlength = data[3];
            let pos = payloadlength + 4;
            const ts = (tsField ? data.readUInt32LE(pos) : new Date().getTime());
            if (tsField) { pos += 4; }
            const rssi = (rssiField ? (8.0/15.0 * data[pos++]) - 380.0/3.0  : -1);
            const crc = (crcField ? data.readUInt16BE(pos++) : 0);
            const payload = data.slice(3, payloadlength + 4);
    ...
    setMode(mode, callback) { // and also disable sleep mode - not saved to nvm
        ...
        this.sendPackage(this.DEVMGMT_MSG_SET_CONFIG_REQ, Buffer.from([0x00, 0x03, 0x00, mode, 0x08, 0x00]), function (res, err) {
            if (err || res.msg_id != that.DEVMGMT_MSG_SET_CONFIG_RSP) {
                that.logError('Error setting link mode ' + mode);
    ...
    init(dev, opts, mode) {
        ...
        this.setMode(m, function(res) {
            if (!res) {
                that.logError('Error setting wMBus mode: ' + mode);
                that.port.close();
                return;
            }
            that.logDebug('Receiver mode set to ' + mode + ' mode');
        });
    }
}
