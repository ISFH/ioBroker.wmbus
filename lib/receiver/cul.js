...
class CUL_WMBUS {
    ...
    onData(data) {
        ...
        if ((that.parserBuffer[len-2] == 0x0D) && (that.parserBuffer[len-1] == 0x0A)) {
            let emitBuffer = Buffer.from(that.parserBuffer.toString('ascii', 1, len - that.CMD_END.length), 'hex'); // remove leading 'b' and trailing \r\n
    ...
    init(dev, opts, mode) {
        ...
        this.sendWithoutReply(this.CMD_DATA_REPORTING_WITH_RSSI, function(res, err) {
            if (err) {
                that.logger("Error setting WMBus reporting: " + (err ? err.message : ''));
                this.port.close();
                return;
            }
