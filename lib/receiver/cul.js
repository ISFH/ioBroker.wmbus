...
class CUL_WMBUS {
    ...
    onData(data) {
        ...
        if (that.parserBuffer.toString('ascii', len-2) == that.CMD_END) {
            let start = (that.parserBuffer[0] == 0x62 ? 1 : 0); // starts with 'b' ?
            let emitBuffer = Buffer.from(that.parserBuffer.toString('ascii', start, len - that.CMD_END.length), 'hex'); // remove leading 'b'? and trailing \r\n
